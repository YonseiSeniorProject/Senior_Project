...
module pe_array#(
    ...
    wire all_pe_done;
    reg all_pe_done_delay;
    wire all_pe_done_rise_edge = (~all_pe_done_delay & all_pe_done);
    
    always @(posedge clk or negedge resetn) begin
        if(~resetn) all_pe_done_delay <= 0;
        else        all_pe_done_delay <= all_pe_done;
    end
    ...
    always @(posedge clk or negedge resetn) begin
     ...
     always @(*) begin
        case (state)
            ...
            COMPUTE : begin 
                if(all_pe_done_rise_edge)   n_state = IDLE;
