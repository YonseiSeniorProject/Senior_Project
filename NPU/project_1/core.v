...
module core#(
    ...
    input wire out_mem_acc_done
    );
    /***** if out_mem_acc_done reset all psum row mems value to zero *****/
    reg [2:0] state, n_state;
    
    reg [ADDR_PSUM-1:0] psum_row_mem_addr_cleaning;
    reg [NUM_COLS-1:0]  psum_row_mem_en_cleaning;
    reg [NUM_COLS-1:0]  psum_row_mem_we_cleaning;
    
    localparam CLEANED   = 3'd0;
    localparam CLEANING  = 3'd1;
    ...
    wire  [NUM_IA_ROW_MEM-1:0]       which_ia_row_mem_activate_2_pe;
//    wire [NUM_WEIGHT_ROW_MEM-1:0]    which_weight_row_mem_activate;

    assign which_ia_row_mem_activate_2_pe = which_ia_row_mem_activate_reg;
    ...
    always @(posedge clk or negedge resetn) begin
        if(~resetn) begin
            ...
            if(data_2_row_mem_done)
                which_ia_row_mem_activate_reg <= which_ia_row_mem_activate;
        end
    end
...
//    wire [(96 * INPUT_BW) - 1:0]            ia_row_mem_doutb_flat;
    ...
    pe_array #(
        ...
        .which_ia_row_mem_activate(which_ia_row_mem_activate_2_pe),
...
//    wire [PSUM_BW-1:0] psum_write_back_data   [NUM_COLS-1:0];
    generate
        for (i = 0; i < NUM_COLS; i = i + 1) begin : gen_psum_write_back_addr
            assign  psum_row_mem_addra[i] = (state==CLEANED) ? top_psum_addr_out_acc[i] : psum_row_mem_addr_cleaning;
        end
    endgenerate
    generate
        for (i = 0; i < NUM_COLS; i = i + 1) begin : gen_psum_write_back_data
            assign  psum_row_mem_dina[i] = (state==CLEANED) ? top_psum_data_out_acc[i] + psum_row_mem_doutb[i] : 0;
        end
    endgenerate
    assign psum_row_mem_ena = (state==CLEANED) ? top_psum_we_acc : psum_row_mem_en_cleaning;
    assign psum_row_mem_wea = (state==CLEANED) ? top_psum_we_acc : psum_row_mem_we_cleaning;
    ...
    assign core_done = (pe_array_done & data_2_row_mem_idle_delay & state==CLEANED);

    /***** if out_mem_acc_done reset all psum row mems value to zero *****/
//    reg [2:0] state, n_state;
    
//    reg psum_row_mem_addr_cleaning;
    
//    localparam CLEANED   = 3'd0;
//    localparam CLEANING  = 3'd1;

//    reg [ADDR_PSUM-1:0] psum_row_mem_addr_cleaning;
//    reg                 psum_row_mem_en_cleaning;
//    reg                 psum_row_mem_we_cleaning;
    always @(posedge clk or negedge resetn) begin
        if(~resetn) state <= CLEANED;
        else state <= n_state;
    end
     ...
     always @(*) begin
        case (state)
            CLEANED : begin 
                if(out_mem_acc_done) begin
                    n_state = CLEANING;
                end               
                else n_state = CLEANED;
            end
            CLEANING : begin
                if(psum_row_mem_addra[0] >= 2048) begin  // PSUM ROW MEM MAX ADDRESS = 2048
                    n_state = CLEANED;
                end
                else n_state = CLEANING;
            end
            default :  n_state = CLEANED;
        endcase
    end
    
    always @(posedge clk or negedge resetn) begin
        if(~resetn) begin
            psum_row_mem_addr_cleaning  <= 0;
            psum_row_mem_en_cleaning    <= 0;
            psum_row_mem_we_cleaning    <= 0;
        end
        else begin
            case (state)
                CLEANING : begin
                    if(psum_row_mem_en_cleaning == 32'hffffffff)
                        psum_row_mem_addr_cleaning  <= psum_row_mem_addr_cleaning + 1;
                    psum_row_mem_en_cleaning    <= 32'hffffffff;
                    psum_row_mem_we_cleaning    <= 32'hffffffff;
                end
                default : begin
                    psum_row_mem_addr_cleaning  <= 0;
                    psum_row_mem_en_cleaning    <= 0;
