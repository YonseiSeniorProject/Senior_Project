...
//            ia_kernel_cnt_delay3    <= ia_kernel_cnt_delay2;
    ...
    // to use DSP
    reg signed [INPUT_BW-1:0]     ia_compute_reg_delay;
    reg signed [INPUT_BW-1:0]     weight_compute_reg_delay;
...
//    reg [5:0] mult_column_cnt;  // multiply is done by ia reuse, when 1 filter width reuse is done, this column cnt increases by 1 
    ...
    always @(posedge clk or negedge resetn) begin
        if(~resetn) begin
            ...
            case (state)
                COMPUTE : begin
                    ia_compute_reg_delay        <= ia_compute_reg;
                    weight_compute_reg_delay    <= weight_compute_reg; 
...
//    wire signed [PSUM_BW-1:0]        mult_data = mult_data_reg;
    wire signed [PSUM_BW-1:0]        mult_data = ia_compute_reg_delay * weight_compute_reg_delay + psum_data_in;
...
//    assign psum_data_out        = (is_psum_addr_equal) ? psum_data_in + mult_data : psum_data_in;   
    assign psum_data_out        = (is_psum_addr_equal) ? mult_data : psum_data_in;
